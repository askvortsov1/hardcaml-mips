...
let gen_testbench () =
  ...
  let step ?(e_dest = "5'h9") ?(m_dest = "5'h9") ?(w_dest = "5'h9")
      ?(e_mem2reg = "1'b0") ?(m_mem2reg = "1'b0") ?(e_rwrite = "1'b1")
      ?(m_rwrite = "1'b1") ?(w_rwrite = "1'b1") () =
    ...
    inputs.controls.w_reg_write_enable := Bits.of_string w_rwrite;
    Cyclesim.cycle sim
  in
  waves, step

let display_rules = [Display_rule.port_name_is "forward_data" ~wave_format:Wave_format.Hex]
...
let%expect_test "Forwards correctly, if should forward" =
  ...
  Waveform.print ~display_rules ~wave_width:4 ~display_width:90 ~display_height:5 waves;
  [%expect
    {|
    ┌Signals───────────┐┌Waves───────────────────────────────────────────────────────────────┐
    │                  ││──────────┬─────────┬─────────┬─────────                            │
    │forward_data      ││ 00000001 │00000002 │00000003 │00000004                             │
    │                  ││──────────┴─────────┴─────────┴─────────                            │
    └──────────────────┘└────────────────────────────────────────────────────────────────────┘ |}]
...
let%expect_test "Doesn't forward for instructions that don't write to registers" =
  ...
  step ~w_dest:"5'h8" ~w_rwrite:"1'b0" ();
...
let%expect_test "Prioritizes which stage forwarding happens if dest is same" =
  ...
  step ~e_dest:"5'h8" ~m_dest:"5'h8" ();
  step ~m_dest:"5'h8" ~w_dest:"5'h8" ();
  step ~e_dest:"5'h8" ~w_dest:"5'h8" ();
  step ~e_dest:"5'h8" ~m_dest:"5'h8" ~w_dest:"5'h8" ();
  ...
  [%expect
    ...
    ┌Signals───────────┐┌Waves───────────────────────────────────────────────────────────────┐
    │                  ││──────────┬─────────┬───────────────────                            │
    │forward_data      ││ 00000001 │00000002 │00000001                                       │
    │                  ││──────────┴─────────┴───────────────────                            │
    └──────────────────┘└────────────────────────────────────────────────────────────────────┘ |}]
